...
advanced_pb2 = importlib.import_module(".proto.advanced_pb2", package=__package__)
...
from .proto.advanced_pb2_grpc import AdvancedServiceStub
...
advanced_stub: AdvancedServiceStub | None = None
...
def get_advanced_stub():
    if advanced_stub is None:
        raise Exception("在建立连接前调用")
    return advanced_stub


def connect(address: str) -> None:
    global grpc_con, listener_stub, playerkit_stub, utils_stub, core_stub, advanced_stub
    ...
    advanced_stub = AdvancedServiceStub(grpc_con)


def load_blob_cache(blob_hash: int) -> bytes:
    response = get_advanced_stub().LoadBlobCache(
        advanced_pb2.LoadBlobCacheRequest(hash=blob_hash)
    )
    if response.status != 0:
        raise RuntimeError(response.error_msg or "LoadBlobCache failed")
    return response.payload if response.found else b""


def update_blob_cache(blob_hash: int, payload: bytes) -> bool:
    response = get_advanced_stub().UpdateBlobCache(
        advanced_pb2.UpdateBlobCacheRequest(hash=blob_hash, payload=payload)
    )
    if response.status != 0:
        raise RuntimeError(response.error_msg or "UpdateBlobCache failed")
    return True
...
def sendcmd_and_get_uuid(cmd: str, request_id: str | None = None):
    ud = request_id or str(uuid.uuid4())
    sendPacket(
...
def sendwscmd_and_get_uuid(cmd: str, request_id: str | None = None):
    ud = request_id or str(uuid.uuid4())
    sendPacket(
        constants.PacketIDS.CommandRequest,
        {
